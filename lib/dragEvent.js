    ...
    window.addEventListener("mousedown", function (mousedownEvent) {
        ...
        (function () {
            ...
            function moveHandler(event) {
                ...
                if (!fired) {
                    fired = true;
                    // use the event's coordinates (clientX/Y, layerX/Y, offsetX/Y) instead of mouseDownEvent's coordinates
                    // this is because the coordinates from mousedownevent lag behind one sample, the first event fired, dragStart, is actually the second event caught: mouseDownEvent -> first mouseMove event
                    // offsetX/Y uses event for another reason
                    dragStartEvent.clientX        = event.clientX;
                    dragStartEvent.clientY        = event.clientY;
                    // mouseDownEvents offsetX and offsetY are always 0
                    // use the moveHandler offsetX/Y coords instead of mouseDown coords
                    dragStartEvent.offsetX        = event.offsetX;
                    dragStartEvent.offsetY        = event.offsetY;
                    dragStartEvent.layerX         = event.layerX;
                    dragStartEvent.layerY         = event.layerY;
                    dragStartEvent.which          = whichKey;
                    dragStartEvent.shiftKey       = mousedownEvent.shiftKey;
                    dragStartEvent.altKey         = mousedownEvent.altKey;
                    dragStartEvent.ctrlKey        = mousedownEvent.ctrlKey;
                    dragStartEvent.preventDefault = mousedownEvent.preventDefault;
